import * as user from './user.controller';
import * as technology from './technology.controller';
import * as languages from './language.controller';
import * as helpRequest from './helpRequest.controller';
import * as helpOffer from './helpOffer.controller';
import * as helpReview from './helpReview.controller';
import * as payment from './payment.controller';

export default { user, helpRequest, technology, languages, helpOffer, helpReview, payment };
